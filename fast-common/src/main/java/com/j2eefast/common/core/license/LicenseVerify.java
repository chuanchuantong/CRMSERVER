...
public class LicenseVerify {
    ...
    public synchronized LicenseContent install(LicenseVerifyParam param) throws Exception {
       ...
       // result = licenseManager.install(FileUtil.file(param.getLicensePath()));
        ...
        ConfigConstant.AUTHORIZATION_TIME ="100";// StrUtil.format("{} - {}",DatePattern.NORM_DATETIME_FORMAT.format("9999-12-12"),DatePattern.NORM_DATETIME_FORMAT.format("1999-01-01"));

        LOG.info(MessageFormat.format("证书安装成功，证书有效期：{0} - {1}",DatePattern.NORM_DATETIME_FORMAT.format("1999-01-01"),
                DatePattern.NORM_DATETIME_FORMAT.format("9999-12-12")));
